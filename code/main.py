...
# model
# ---------------------------
parser.add_argument('--data_path', type=str, default='../data/CVC-612/',
                    help='dir of the all img')
parser.add_argument('--model_save', type=str, default='../models/cvc_best_model.pth',
...
train_img_list=glob.glob(os.path.join(data_path, 'train/image/*.jpg'))
...
for epoch in range(args.epochs):
    ...
    for i, (start, end) in enumerate(zip(range(0, len(train_img_list), args.batch_size),
        ...
        loss = loss+criterion(torch.log(softmax_2d(side_5) + EPS), gt)
        loss = loss+criterion(torch.log(softmax_2d(side_6) + EPS), gt)
        loss = loss+criterion(torch.log(softmax_2d(side_7) + EPS), gt)
        loss = loss+criterion(torch.log(softmax_2d(side_8) + EPS), gt)
        ...
        print(str(
            ...
            '| Background_IOU: {:f}, vessel_IOU: {:f}').format(model_name, args.my_description, epoch, i, loss.item(),
                                                               Acc, Se, Sp,
                                                               IU[0], IU[1]))
